...
suite('Extension Test Suite', () => {
    ...
    return [..quicSort(left), pvot, ...qickSort(right)];
        ...
        // Trigger the completion command
        await vscode.commands.executeCommand('code-assistant.debugWithCodeBuddy');
    ...
    test('TC_007 - Debug Code with No Error', async function () {
        // Create a temporary file
        const filePath = path.join(os.tmpdir(), 'tc007.js');
        fs.writeFileSync(filePath, 
            `function quicksort(arr) {
                if (arr.length <= 1) {
                    return arr;
